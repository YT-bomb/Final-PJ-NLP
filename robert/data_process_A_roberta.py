...
for id, sents in train_data.items():
    ...
    train_sample.append((templates[template_id].format(sents[true_id]),1))
    train_sample.append((templates[template_id].format(sents[false_id]),0))

random.shuffle(train_sample)
f1 = open("SemEval/TaskA/data/train.input0", 'w')
f2 = open("SemEval/TaskA/data/train.label", 'w')
...
f1 = open("SemEval/TaskA/data/dev.input0", 'w')
f2 = open("SemEval/TaskA/data/dev.label", 'w')
for sample in train_sample:
    f1.write(sample[0] + '\n')
    f2.write(str(sample[1]) + '\n')
f1.close()
